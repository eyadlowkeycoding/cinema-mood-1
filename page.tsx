...
export default async function SeriesPage({ searchParams }: { searchParams: Promise<{ genre?: string; country?: string }> }) {
  const { genre, country } = await searchParams;

  const [trending, allTime, topRated, popular, hiddenGems, recent, short, long, egyptian, ...genreRows] = await Promise.all([
    getTrending("tv", 16),
  ...
  return (
    <div className="pb-24 pt-28 md:pb-16">
      ...
      <MediaCarousel title="🇪🇬 Egyptian Series" items={egyptian} />
      {GENRE_ROWS.map((g, i) => (
      ...
      <section className="px-5 py-10 sm:px-8">
        ...
        <BrowseSection type="tv" initialGenre={genre} initialCountry={country} />
      </section>
    </div>
  );
}
