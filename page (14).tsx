...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const item = await getDetails("tv", Number(id));
...
export default async function TvDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const providers = await getWatchProviders("tv", item.id);
  ...
  return (
    <div className="pb-24 md:pb-16">
          ...
          <div className="flex-1">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-4 font-mono text-sm text-paper-dim">
              ...
              {item.numberOfSeasons && (
                <span className="flex items-center gap-1"><Layers size={14} /> {item.numberOfSeasons} season{item.numberOfSeasons > 1 ? "s" : ""}</span>
              )}
              {item.numberOfEpisodes && <span>{item.numberOfEpisodes} episodes</span>}
            </div>
            <p className="mt-5 max-w-2xl text-balance text-paper-dim">{item.overview}</p>
            <div className="mt-6 flex flex-wrap gap-6 text-sm">
              <div><span className="text-paper-faint">Creator</span><div className="text-paper">{item.director}</div></div>
              {item.cast.length > 0 && (
      ...
      {item.seasons && item.seasons.length > 0 && (
        <section className="mx-auto max-w-6xl px-5 py-8 sm:px-8">
          <h2 className="mb-4 font-display text-lg font-semibold text-paper">Episodes</h2>
          <SeasonSelector tvId={item.id} seasons={item.seasons} />
        </section>
      )}

      {item.similar.length > 0 && <MediaCarousel title="Similar Series" items={item.similar} />}
    </div>
  );
}
