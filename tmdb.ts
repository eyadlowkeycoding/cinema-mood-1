...
function normalizeTmdb(raw: RawTmdbItem, type: MediaType): MediaItem {
  ...
  return {
    ...
    country: raw.origin_country?.[0] ?? raw.production_countries?.[0]?.iso_3166_1 ?? null,
...
export interface DiscoverParams {
  genre?: string;
  year?: number;
  language?: string; // ISO code, e.g. "ar"
  country?: string; // origin country ISO code, e.g. "EG"
  minRating?: number;
  sort?: "rating" | "popularity" | "newest";
  page?: number;
}

export async function getByCountry(type: MediaType, country: string, limit = 20, sort: "popularity" | "rating" = "popularity"): Promise<MediaItem[]> {
  return (await discover(type, { country, sort })).slice(0, limit);
}
...
export async function discover(type: MediaType, params: DiscoverParams = {}): Promise<MediaItem[]> {
  ...
  if (data) {
    ...
    if (params.country) results = results.filter((m) => m.country === params.country);
